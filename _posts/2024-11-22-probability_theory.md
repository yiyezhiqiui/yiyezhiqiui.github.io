...
## 条件概率
条件概率:事件A已发生的条件下事件B发生的概率$P(B|A_1)=P(AB)/P(A)$

全概率公式:试验E的样本空间为S,A为E的事件,$B_1,B_2……B_n为S的一个划分,且P(B_i)>0(i=1,2……,n)则$

$P(A)=P(A|B_1)P(B_1)+P(A|B_2)P(B_2)+……+P(A|B_n)P(B_n)$为全概率公式

贝叶斯公式:试验E的样本空间为S,A为E的事件,$B_1,B_2……B_n为S的一个划分,且P(A)>0,P(B_i)>0(i=1,2……,n)则$

$P(B_i|A)=\frac{P(A|B_i)P(B_i)} {\sum_{j=1}^n P(A|B_j)P(B_j)}$为贝叶斯公式
