...
$$(1)当相位差\phi_{20}-\phi_{10}=0时，可得

\frac{x}{A_1}=\frac{y}{A_2}$$

$$(2)当相位差\phi_{20}-\phi_{10}=\pm\pi时，可得
...
$$观察者以速度v_s相对于介质运动，接收到的频率为\upsilon_R=\frac{u+v_R}{u}\upsilon_s$$
...
### 波源和观察者在同一直线上运动
$$相向而行v_R=\frac{u+v_R}{u-v_s}\upsilon_s\\

彼此分开v_R=\frac{u-v_R}{u+v_s}\upsilon_s$$

## 波的衍射和干涉
$$\Delta\phi=\pm2k\pi时，干涉加强\\

\Delta\phi=\pm(2k+1)\pi时，干涉减弱$$
...
## 光程
$$光程：在相同时间内，光在介质n中传播距离r引起的相位变化，计算方法为相位差与光程差的关系为\Delta\phi=\frac{2\pi}{\lambda}\delta$$

## 杨氏双缝干涉
$$\frac{\delta}{d}=\frac{x}{D}$$

$$\delta为光程差，d为双缝间的距离，x为干涉条纹到双缝垂直平分线的距离，D为双缝到光屏的距离$$
...
$$
明环的半径为r=\sqrt{(2k-1)\frac{R\lambda}{2n}}\\

暗环的半径为r=\sqrt{k\frac{R\lambda}{n}}\\

圆环间距为a=\frac{\lambda}{2sini}\\
$$
...
单缝宽度a越小，入射光波波长\lambda越大，明纹的角宽度和线宽度越大，衍射越明显\\

透镜的位置不变而使单缝的位置上下微笑移动，衍射图样的位置不变\\

复色光入射，中央0级明条纹中心仍为复色光的混合，其他级明条纹呈现彩色。\lambda越小的色条越靠近中心位置。高级次彩色条纹将出现重叠。\\
$$
### 光学仪器的分辨本领
$$
...
满足d\sin\phi=\pm k\lambda,k为明条纹级数\\

缺级条纹的k_1和明条纹二点k的关系为k_0=\frac{a+b}{a}k\\
